...
`--skip-unchanged` skips the LLM call entirely when the consolidatable memory set is unchanged since the last applied consolidation: a fingerprint over the consolidatable set's prompt-visible fields (access counts excluded) is stored after each successful `--apply`, and a matching fingerprint exits before any model call. The auto lifecycle passes it; manual runs without the flag are unchanged.
