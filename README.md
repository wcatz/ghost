...
PRs are reviewed automatically by two workflows, powered by Big Pickle through the opencode CLI. Every path is in scope, docs included — a prose diff is reviewed as documentation (claims the code contradicts, dead references, internal contradictions) rather than as Go.
...
**`sweeper.yml`** runs after every Reviewer completion, whatever its conclusion, and publishes the `review-gate` check run. A stale thread no longer blocks anything: the gate counts only threads that are unresolved **and** not outdated. It still attempts to resolve stale threads, but only after a review that actually judged the commit — a skipped or failed review produced no judgement, and a thread resolved on its strength would be fed back to the model as dismissed and never re-raised. It does not post `REQUEST_CHANGES`: an outstanding changes-requested review blocks merge even with zero required approvals, and the bot cannot dismiss its own review any more than it can resolve its own thread — so that would be a second gate it could never clear.

**Review identity:** reviews post as the Review Loop GitHub App (`review-sweeper`) when the app token is available, falling back to `github-actions`. Neither identity can resolve a review thread — see the gating note above — so nothing in the merge path depends on that. `GITHUB_TOKEN` also cannot submit `APPROVED`, and it deliberately does not submit `REQUEST_CHANGES`. What it can do is publish the check run that does the gating, which is the whole point: the gate is the one thing in this pipeline the bot owns outright.
