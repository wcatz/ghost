...
- **The staleness suite** ("prod ran Postgres 14, we migrated to 16" — does search rank the fresh fact first?) runs report-only in CI. At the shipped default, fresh facts are always retrieved but outrank their superseded versions only 8% of the time — a failure no memory system we know of even measures. An off-by-default recency prior (`SearchParams.RecencyWeight`) flips it to 100% in the sweep — but we then built a *recency-trap* fixture (older memory is the correct answer) and proved the global prior can't be a default: it's a cliff, not a tradeoff, and every weight that fixes staleness destroys old-but-still-correct retrieval. So the prior stays off, and the real fix — LLM-classified `supersedes` links that demote a memory only when its actual replacement co-occurs — is the roadmap. Publishing the negative result *and* the reason is the point.
