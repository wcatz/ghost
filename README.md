...
- **The staleness suite** ("prod ran Postgres 14, we migrated to 16" — does search rank the fresh fact first?) runs report-only in CI. At the shipped default, fresh facts are always retrieved but outrank their superseded versions only 8% of the time — a failure no memory system we know of even measures. The fix (an off-by-default recency prior, `SearchParams.RecencyWeight`) is now in the tree and **flips the suite to 100% in the sweep** while leaving production ranking byte-identical; it stays off until a recency-trap fixture proves it won't demote old-but-still-correct facts. The suite is the gate for that roadmap.
