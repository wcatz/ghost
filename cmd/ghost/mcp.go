...
func runMCPStatus() {
	...
	// Printed before the unhealthy exit, and on a store this command opened
	// read-only: a project with no usable path and no repository remote is not
	// a health failure — every check above can pass while sessions in that
	// checkout silently get no context — so it is advice, and advice must
	// survive a non-zero exit. It also must not be a bootstrap() call, because
	// bootstrap creates the database, stamps its schema version and seeds the
	// builtin rows: a status check that made those writes would turn the next
	// run's "no Ghost database (run ghost first)" into a healthy line, so the
	// diagnostic would invalidate its own result. A failure to read the
	// projects is warned about rather than fatal for the same reason the
	// notice is printed before the exit: the checks above already reported.
