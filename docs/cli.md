...
One case needs action before init will write anything: `writeRedirects` decides by file content, not by who wrote the file. It skips a `MEMORY.md` that does not already look like a Ghost redirect, and rewrites one that does when it still carries the stale `ghost_list_projects` marker — so a checkout whose own `MEMORY.md` mentions Ghost can be replaced. Merge or remove such a file first. `mcp status` counts a skipped project as not redirected and prints the same line as any other failure, so the bind output states the condition init uses rather than promising a redirect that will not appear. The bind output names the installation the step belongs to as well.
