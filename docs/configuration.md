...
- OpenCode receives `OPENCODE_API_KEY` when configured. If authentication is file-based, Ghost copies only the existing `auth.json` into the invocation-owned data directory; the child still uses an invocation-owned home/config tree and an ask-every-tool policy with no MCP servers or plugins (a non-interactive `opencode run` declines every ask; a deny policy would strip tools from the request, which OpenCode's free tier rejects with 403), so Ghost does not load the user's OpenCode config or plugins.
