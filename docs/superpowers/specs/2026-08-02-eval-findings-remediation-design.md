...
**Test plan:** unit tests on `Upsert` asserting original content is never destroyed on match (currently would need new coverage — check `store_test.go` for existing dup-merge tests to extend, not replace). Verify `source='manual'` targets no longer silently drop new content.
