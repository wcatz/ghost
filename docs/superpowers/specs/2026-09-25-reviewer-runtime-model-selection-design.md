...
The reviewer is intentionally keyless, but the anonymous OpenCode free tier changes
frequently. A model can be removed from the catalog, reject the read-only custom agent
with `FreeTierError`, or hit a rate limit. Replaying a fixed list wastes review time and
can fail before a currently usable model is tried.
...
The first passing candidate is selected for the one real review invocation. The workflow
logs the selected model and logs every considered candidate that was skipped, with the
reason. Provider/model text is normalized and percent-escaped before it is placed in a
GitHub Actions workflow command. The post-review command and trusted script boundary are
unchanged.

## Safety invariants

- No API key is introduced; the model phase remains credential-free.
- The agent file remains in `$HOME/.config/opencode/agent` and keeps explicit read-only
  tool permissions.
- Every OpenCode catalog/probe command runs from `$RUNNER_TEMP`, not the PR workspace.
- A missing agent is a hard error.
