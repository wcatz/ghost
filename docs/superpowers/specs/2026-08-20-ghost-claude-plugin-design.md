...
- **Release CI:** goreleaser builds the six binaries and assembles `ghost-plugin.zip`; a step computes and pins the `sha256` in `marketplace.json`; PR CI runs `claude plugin validate --strict` against the assembled trees and `go vet ./...`.
