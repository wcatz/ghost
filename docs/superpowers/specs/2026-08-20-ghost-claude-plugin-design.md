...
6. The updated `marketplace.json` is committed to `main` as a bot-signed `chore(release)` commit — measured blocked for `GITHUB_TOKEN` at v0.30.18 (contents-API PUT → HTTP 409 GH013, run 35928382388; a git-push probe under the same token, run 35929704681, was rejected with the identical rule violations — the `main-branch-protection` bypass actor User 41898282 `github-actions[bot]` does not cover a workflow token, whose effective principal is the GitHub Actions app integration and cannot be a bypass actor on a personal repo), so the pin is completed by the manual admin runbook (`scripts/update-marketplace-digest.sh` against the published assets + a bypassing admin push to `main`) until an authorization route (PAT secret / owner-merged PR / accept the manual pin) is chosen — skipped when main already matches (re-runs of the same release are idempotent given reproducible archives) and skipped behind a downgrade guard when main already pins a newer version, so an older release's re-run never rolls the catalog back; the `GITHUB_TOKEN` push does not trigger follow-on workflow runs.
