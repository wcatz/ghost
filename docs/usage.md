...
A save from a directory *inside* a project's checkout never speaks for that project. If the directory is the checkout itself, or belongs to the same repository, the save identifies the project and binds the remote as usual — that is what a session started in `src/api` reports, and what a second worktree of the same repository reports. A checkout *inside* the project that is a repository of its own — a submodule, a vendored clone — describes itself, not the project around it, so its remote is not recorded there. When the project records no remote yet, the save lands in it and leaves it unclaimed; when it already records a different one, the save is refused with `project "..." belongs to a different repository`, which is the same conflict rule that applies to any other contradicting identity. Either way the enclosing project keeps its own identity, and the checkout that caused the trouble gets a project of its own from `ghost project bind`, which states which checkout is which project rather than letting a save decide it.
