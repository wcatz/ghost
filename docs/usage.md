...
A project can reach that state without ever being created over MCP. Databases written before `repo_remote` existed (schema v11) recorded the bare project name as the project's path rather than a real location, and they stayed that way when #565 removed the directory-resolution fallback to a project's *name* — the fallback that let an unrelated clone parked in a similarly named folder read and inject another project's memories (#546). So those projects upgraded into a state no directory can resolve: they are still found by name when you ask for one by name, but a session standing in the checkout reports no match and gets no context or lifecycle work. `ghost mcp status` lists every project in that state with the command that repairs it:
...
Binding records the directory, and the checkout's Git remote when the project records none, so worktrees and moved checkouts of the same repository keep resolving to the same project. It refuses a path or repository another project already claims, because the repair is meant to be explicit — see the [CLI reference](cli.md#project-operations).
