...
A project can reach that state without ever being created over MCP. Upgrading to schema v16 stopped directory resolution from falling back to a project's *name*, because that fallback is what let an unrelated clone parked in a similarly named folder read and inject another project's memories (#546, #565). Databases written before that change stored the bare project name as the project's path rather than a real location, so those projects upgraded into a state no directory can resolve — they are still found by name when you ask for one by name, but a session standing in the checkout reports no match and gets no context or lifecycle work. `ghost mcp status` lists every project in that state with the command that repairs it:
