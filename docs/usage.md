...
A save from a directory *inside* a project's checkout — a submodule, a vendored repository — still lands in that project, but never speaks for it. The Git remote found in a nested checkout describes the nested checkout, not the project that encloses it, so it is not recorded on that project, and a remote the project already records is left alone. Recording it would lock the enclosing checkout out of its own project everywhere except exactly its root. A nested checkout that is meant to be its own project is given one by `ghost project bind`, which states which checkout is which project rather than letting a save decide it.
