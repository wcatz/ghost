...
| **Validity** | Is this memory true *now*, and when was it last checked? | `memories.valid_from`, `valid_until`, `verified_at` | Inert — the columns exist but are neither written nor read on the main paths ([#575](https://github.com/wcatz/ghost/issues/575)); wiring them into retrieval is part of the assembler ([#581](https://github.com/wcatz/ghost/issues/581)) |
...
- **Resolved leaves injection, not the database.** `resolved_at` removes a row from assembled context and from search results ([#559](https://github.com/wcatz/ghost/issues/559)) but keeps it searchable and auditable.
