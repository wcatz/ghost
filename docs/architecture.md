...
Every other open — `OpenDBReadOnly`, the `roDSN` paths in `hook.go`, `stophook.go`, `lifecyclelock.go` and `lifecyclemarker.go`, `obsidian.go`, and `eval/cycle` — is read-only and deliberately stays that way: a diagnostic must be able to report on a database it cannot modify, and a read-only connection cannot create one.

Note that "read-only open" is a property of the open, not of the command. Both read-write opens sit behind `cmd/ghost`'s shared `bootstrap()`, which runs migrations and stamps `user_version` before any command-specific work, so most commands reach the database through `OpenDB` whether or not they write: `ghost mcp status` and `ghost maintenance status` do so through `mcpinit.checkStoreHealth` and `RecentMaintenanceRuns`, and `ghost project bind` through `bootstrap` before `BindProjectPath`. A session stop can reach it indirectly too, when the stop hook spawns `ghost lifecycle` for a reflection pass. That was already true of the migrations those paths run; the modes follow the same route, so the pass adds no new place where a database is written.
