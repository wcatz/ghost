...
Every other open — `OpenDBReadOnly`, the `roDSN` paths in `hook.go`, `stophook.go`, `lifecyclelock.go` and `lifecyclemarker.go`, `obsidian.go`, `project.go`, and `eval/cycle` — is read-only and deliberately stays that way: a diagnostic must be able to report on a database it cannot modify, and a read-only connection cannot create one. Note that "read-only open" is a property of the open, not of the command: `ghost mcp status` and `ghost maintenance status` reach the database through `OpenDB` (via `mcpinit.checkStoreHealth` and `RecentMaintenanceRuns` respectively), so they tighten. That is pre-existing — they already ran migrations and stamped `user_version` to print a health line.
