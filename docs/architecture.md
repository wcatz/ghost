...
Two of the user-owned config writes, the Claude `settings.json` and the codex `config.toml`, go through one atomic path (temp file in the same directory, then rename, under a random hidden name), so an interrupted write cannot leave a half-written host config, and a symlinked config is written through to its target rather than replaced by it. A file that already exists keeps its own permissions on the codex side, but `settings.json`, which can hold hook commands and env, is clamped so it can never be group- or world-readable whatever mode the user's copy had. That is the ceiling every save enforced before the refactor; a copy narrower than `0600` is now left at its own narrower mode rather than widened to `0600`. The remaining user-owned writes (codex `hooks.json`, the opencode plugin, the goose package, the MEMORY.md redirect) still use a bare `os.WriteFile` and are tracked in #551. The Claude `settings.json.bak` backup is captured once, by the first save that finds a file to back up, so repeated `init` runs never overwrite the pristine original with ghost's own output.
...
`reflect` replaces non-manual/non-builtin memories through a tiered consolidator. It snapshots before replacement, rejects empty results, preserves manual and Ghost-shipped builtin memories, and can restore the latest snapshot. Before replacement, every input memory is audited for a surviving merge target, in all eight categories: an input whose tokens are less than 45% contained in any single output memory is re-added verbatim rather than deleted, so consolidation merges only what it can show it absorbed. `--allow-drops` accepts the deletions instead of retaining them. A memory re-emitted byte-identically in the same category keeps its row identity, its `created_at` and its `source` — retention is not consolidation, so it does not restart the decay clock of a memory the consolidator omitted as stale, nor overwrite the provenance (say, `mcp`) that recorded where it came from. A memory reflection actually rewrote — new text, or the same text in a different category, since category is half of what decay ranks on — counts as refreshed knowledge and gets the new timestamp and `source = 'reflection'` instead. Which row a re-emission reuses is decided, not incidental: a project can hold one text under several categories (`Upsert` keeps the incoming category on a linked copy rather than recategorizing the existing row), so a same-category row is preferred, and duplicate rows are considered oldest first, so the age that survives a consolidation is the oldest one and the same corpus always consolidates to the same result. Cross-project candidates stay in the project by default; `--promote-globals` opts into `_global`, and the project replacement plus any global writes commit in one store transaction. `resolve` stamps resolved evidence so it leaves injection but remains searchable. `supersede` creates directed replacement links after source-matched classification.
