...
"Read-only" is a property of the open, not of the command, and a read-write open tightens wherever it is called. `cmd/ghost`'s shared `bootstrap()` is the usual route, and it already ran migrations and stamped `user_version` before any command-specific work, so most commands tighten simply by starting up. Three paths open read-write without it, and each was already writing or migrating for its own reasons: `mcpinit.checkStoreHealth` (behind `ghost mcp status`, which must not bootstrap — a status check that created the database would turn the next run's "no Ghost database" line into a healthy one), `runMaintenanceStatus`, and the hook path, where `runSessionStart` and `RenderSessionContext` reach `bumpSessionCount` under `ghost hook` / `ghost context`. That last one is narrower than it looks. `runSessionStart` returns early for a subagent, a `resume` and a `compact` fire, and gates the bump on `projectID != ""` and on the source being empty or `startup` — so a `clear` does not bump either. `RenderSessionContext`, the opencode plugin's startup entry point, has no source or subagent gate, but is likewise gated on `projectID != ""`. So a genuine new session in a directory that resolves to a project tightens; a resumed, cleared or compacted one does not. That is the same gate that keeps the session counter honest. A session stop can reach a read-write open indirectly when the stop hook spawns `ghost lifecycle` for a reflection pass.
