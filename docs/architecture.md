...
`reflect` replaces non-manual/non-builtin memories through a tiered consolidator. It snapshots before replacement, rejects empty results, preserves manual and Ghost-shipped builtin memories, and can restore the latest snapshot. Before replacement, every input memory is audited for a surviving merge target, in all eight categories: an input whose tokens are less than 45% contained in any single output memory is re-added verbatim rather than deleted, so consolidation merges only what it can show it absorbed. `--allow-drops` accepts the deletions instead of retaining them. Cross-project candidates stay in the project by default; `--promote-globals` opts into `_global`, and the project replacement plus any global writes commit in one store transaction. `resolve` stamps resolved evidence so it leaves injection but remains searchable. `supersede` creates directed replacement links after source-matched classification.
...
There are exactly **two** read-write opens in the tree, and both call it. `memory.OpenDB` is the one that creates the database or migrates it, and calls the pass itself. `mcpinit.bumpSessionCount` is the session hook's single deliberate write, over its own short-lived `rwDSN` connection, and calls it after its existence check — a loose mode left by an older build is still loose when the session hook writes, since no other Ghost process may have opened the database in between. Every other open (`OpenDBReadOnly`, the `roDSN` paths in `hook.go`, `stophook.go`, `lifecyclelock.go`, `lifecyclemarker.go`, `obsidian.go`, `project.go`, and `eval/cycle`) is read-only and deliberately stays that way: a diagnostic must be able to report on a database it cannot modify, and a read-only connection cannot create one. That is why the pass is a named function rather than a line inside `OpenDB` — a read-only path is not a place to add a side effect to, but a read-write one is.

The contract guards are `internal/memory/perms_test.go` and `TestBumpSessionCountTightensPermissions` / `TestBumpSessionCountNoPhantomDB` in `internal/mcpinit/hook_test.go`; see [`configuration.md`](configuration.md#data-directory-permissions) for the user-facing description.
