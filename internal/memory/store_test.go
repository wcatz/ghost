...
func TestStoreReplaceNonManual(t *testing.T) {
	...
	t.Run("preserves memories saved concurrently with consolidation", func(t *testing.T) {
		...
		// Simulate ghost reflect's flow: capture "now" before consolidation
		// starts, then something lands on the live MCP server (ghost_memory_save)
		// while the (slow) consolidation round trip is in flight.
