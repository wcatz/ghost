...
func TestMigrateV14RelabelsBuiltinSeed(t *testing.T) {
	...
	if _, err := db.Exec(`
		...
		VALUES ('_global', 'preference', 'NEVER add Co-Authored-By or any AI attribution to commit messages. All commits belong to the user.', 'manual', 1.0, '[]', 1)
	`); err != nil {
	...
	if source != "builtin" || !pinned {
		t.Errorf("migrated seed = source %q pinned %t, want builtin/pinned", source, pinned)
	}
}
