...
// platform Ghost user config path and ~/.local/share/ghost by default. Without
...
func TestBumpSessionCountNoPhantomDB(t *testing.T) {
	...
	// Nothing was written, so nothing about the directory should have changed
	// either. Had the permission pass run before the existence check, a session
	// hook on a machine with no Ghost database would still have found a way to
	// make something appear in the data directory — here, a directory a test
	// asserted was still group-writable on a path where nothing has ever
	// created one. The pass must not become a directory-touching side effect of
	// a function whose contract is "never create anything".
	dataHome := t.TempDir()
...
func TestBumpSessionCountTightensPermissions(t *testing.T) {
	...
	// A store that already exists, at the width #553 reported. memory.OpenDB
	// would fix this on its next call, so the write here is what has to fix it
	// for a database nothing else has opened since.
