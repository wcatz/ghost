...
// added to this seam so far is dry-run-restricted; the flag exists so a
// future trusted secondary can opt out explicitly rather than silently
...
func (f *FallbackProvider) Classify(ctx context.Context, systemPrompt, userContent string) (ClassifyResult, error) {
	...
	if f.secondary == nil || !isCreditExhausted(err) {
