...
- Classifier fallback: `resolve`/`supersede` classify via `ai.FallbackProvider` — Anthropic primary, falls to a secondary only on `ai.ErrCreditExhausted`. The headless CLI path (`ghost resolve`/`ghost supersede`, and the stop hook's auto-resolve) wires no secondary, so it fails fast with a clear message on credit exhaustion instead of degrading. The live-session `ghost_resolve` MCP tool prefers MCP sampling (the calling session's own model, no API credits spent); when the client doesn't support sampling it falls back to a subscription-billed `claude -p` CLI call (PR #312). A fallback answer never auto-applies a write (`--apply` is skipped with a log line) — see `docs/superpowers/plans/2026-07-26-classifier-fallback.md`.
