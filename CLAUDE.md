...
- `internal/mcpinit/` — `ghost mcp init`, `ghost mcp status`, `ghost hook <event> --source <host>` (contract-v1 lifecycle dispatch; opencode integration is a single embedded plugin that self-registers MCP + bridges stop events)
