...
- CLI harness callers (claude/opencode/codex/goose subprocesses, subscription-billed) — used by reflection, resolve, and supersede. `cli.model_reflect` / `model_resolve` / `model_supersede` pin the opencode harness model per phase (each phase is its own process; empty = harness default, ignored by claude/codex/goose which have no model flag).
